...
function charaFormatData(data){
    var char = {"name": data.ch_name, "description": data.description, "absolute_rule": data.absolute_rule, "skip_absolute": data.skip_absolute, "personality": data.personality, "first_mes": data.first_mes, "avatar": 'none', "chat": Date.now(), "mes_example": data.mes_example, "scenario": data.scenario, "create_date": Date.now()};
    return char;
}
...
app.post("/getbackgrounds", jsonParser, function(request, response){
    ...
    if(is_colab === true){
        images = ['1.png', '2.png', '3.png', '4.png', 'tavern.png'];
    }
            ...
            if(format == 'json'){
                fs.readFile('./uploads/'+filedata.filename, 'utf8', (err, data) => {
                    ...
                    if(jsonData.name !== undefined){
                        ...
                        let char = {"name": jsonData.name, "description": jsonData.description ?? '', "absolute_rule": jsonData.absolute_rule ?? '',"skip_absolute": jsonData.skip_absolute ?? '', "personality": jsonData.personality ?? '', "first_mes": jsonData.first_mes ?? '', "avatar": 'none', "chat": Date.now(), "mes_example": jsonData.mes_example ?? '', "scenario": jsonData.scenario ?? '', "create_date": Date.now()};
                        char = JSON.stringify(char);
                        charaWrite('./public/img/fluffy.png', char, png_name, response, {file_name: png_name});
                    }else if(jsonData.char_name !== undefined){//json Pygmalion notepad
                        png_name = getPngName(jsonData.char_name);
                        let char = {"name": jsonData.char_name, "description": jsonData.char_persona ?? '', "absolute_rule":jsonData.absolute_rule ?? '',"skip_absolute": jsonData.skip_absolute ?? '',"personality": '', "first_mes": jsonData.char_greeting ?? '', "avatar": 'none', "chat": Date.now(), "mes_example": jsonData.example_dialogue ?? '', "scenario": jsonData.world_scenario ?? '', "create_date": Date.now()};
                        char = JSON.stringify(char);
                        charaWrite('./public/img/fluffy.png', char, png_name, response, {file_name: png_name});
                    }else{
